...
> **历史审查记录，不是当前缺陷清单。** 下文保留 2026-04-14 的原始判断、行号和方案；其中“当前”“未提供”等措辞仅指当时的版本。条目可能已修复、部分迁移或被后续方案替代，不能据此直接删除代码或重复实施。
>
> 当前入口、职责和迁移边界见 [Agent 维护指南](AGENT_MAINTENANCE.md)。处理历史条目前，应在当前分支重新核对实际调用、Git 状态及模块测试；历史报告和通过数量不代替本次验证。
