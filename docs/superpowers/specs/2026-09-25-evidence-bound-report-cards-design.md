...
Status (updated 2026-09-25): **G1 implementation was dual-approved and integrated through PR74 at local HEAD `400beaf`; PR77 then encountered a real root CI failure. Parent-authorized one-literal test correction is locally GREEN and uncommitted, frozen for the original QUALITY reviewer.** See the [current correction evidence and commands](../plans/2026-09-25-evidence-bound-report-cards.md#pr77-root-ci-correction--uncommitted-quality-freeze-at-400beaf) and historical [freeze handoff](../../handoff/2026-09-25-g1-evidence-report-freeze.md). CI run `36069659921` / job `107867398126` remains a recorded failure, not a flaky timing result or an already-green rerun. Prior Agent-only full did not cover this root test. PR76/main `5ad08ac` is deliberately not merged in this correction. Original design-stage baseline/status statements below are historical. No dynamic/P7 completion is claimed.
...
- Root template-loading acceptance is mandatory in addition to Agent/Node tests. Through the existing isolated RAG runner (repo path replaced by this worktree), run the complete `tests/test_static_placeholder_cleanup.py` module with `-q -p no:cacheprovider --tb=short -rs`; do not infer its success from `tests/agent`. Its exact `PAGE_SCRIPTS` home list must include `scientific_references.js`, then `evidence_report.js`, then `formatters.js`, with `main.js` later. Preserve the full ordering, every local URL 200, forbidden/placeholder script, worker isolation/network and cleanup assertions. Run the existing five Node commands in the plan; `node tests/home_workflow_completion_behavior_test.js` already includes the original homepage main-script cache-token assertion and must retain it unchanged.
- PR77 correction evidence: the unchanged root module reproduced **1 failed / 6 passed, 18.13s** at `400beaf`; child line 56 compared 12 actual scripts with an 11-entry expected list, parent line 128 merely forwarded the failure. Sole difference was the already-shipped `evidence_report.js` after `scientific_references.js`; no runtime dependency/order change was needed. Parent approved adding just that literal at test line 38. After the patch, the entire module is **7 passed, 17.89s**, and all five Node scripts pass. All function/class ASTs and the 16 existing G1 non-doc file fingerprints remain unchanged. This is local verification, not a CI rerun or QUALITY approval; no commit/push/full/merge is authorized for this correction.
