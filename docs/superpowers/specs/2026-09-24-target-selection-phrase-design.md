...
日期：2026-09-24。用户已确认最小方案及书面设计，授权进入 TDD。
...
- 句尾可有空白和一个英文/中文句末标点（`.`、`!`、`?`、`。`、`！`、`？`）。
- 完整短语（及可选标点）之后的尾部空白允许换行，保持 Router 既有 `strip()` 与 Planner 一致；连接符、select/top/N 之间仍不跨行。换行后有额外正文不放行。
...
本修复发布前应在 PR #53 获具体授权合并后对齐 main，再将该测试明确改为本次新行为，并保留其余全字段迁移断言。
用户确认进入 TDD 后，本分支推进本地独立实现；不提前改变 PR #53 的已审快照，也不让两个互相矛盾的断言同时发布。
