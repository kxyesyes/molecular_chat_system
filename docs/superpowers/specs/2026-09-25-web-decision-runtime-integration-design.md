...
Date: 2026-09-25. Status: **G0 and the revised worker-ownership/history design text approved by parent; explicit staging and a local documentation-only commit of this spec/plan pair are authorized. G1 remains blocked pending corrected, approved and landed A1 plus separate parent release. Not implementation authorization, not A1 quality approval, not P7 completion.**
...
Parent update: Planner #71 is merged into main at `5db56b0`. This is parent-supplied integration context, not a revision merged or executed here. The original design base remains `bb11ded`; this revision edits only the two documents from planning HEAD `ff02afc`, initially clean. The preceding read-only risk review compared main at `9a9eb8d` and A1 at `7a239b5` plus dirty admission/spec/plan/tests. That A1 working copy is a dependency observation only, not landed or approved evidence. No merge/rebase occurs here. Later A2 must pin reviewed A1 fixes and the then-current approved integration base, including Planner #71 compatibility.

Authority: repository `AGENTS.md`, `docs/PROJECT_STANDARDS.md`, and this task's narrower instructions. Parent has reviewed and approved the revised text; only this spec and `docs/superpowers/plans/2026-09-25-web-decision-runtime-integration.md` may receive approval-status updates, be explicitly staged and committed locally. This supersedes the preceding uncommitted re-review hold. No production/test changes, imports, tests, compilation, network, environment/credential inspection, services, merge or push are authorized. Future code/test allowlists below are not present execution authority. Packages 1–8 remain the parent's goal; deployment is excluded. No parent goal/ledger status is changed here.
...
| `harness/decision_execution.py::settle_action`; `orchestrators/workflow.py::_execute_step`; `tooling/adapters.py::_execute_once` | At main `9a9eb8d`, `settle_action` lines 54–78 only joins the outer Session call. Workflow lines 149–168 and adapter lines 127–166 can return timeout with `shutdown(wait=False)` while nested invocation threads remain alive. Neither a Session timeout result nor a cancelled asyncio task proves physical drain; section 6.1 supplies the missing ownership contract |
...
The later read-only A1 observation (`7a239b5` plus dirty files) deliberately restricts ordinary chat: `decision_request.py` lines 86–95 admit closed nominal explanations; lines 117–120 admit complete greetings and reject other free text. `tests/agent/test_web_decision_admission.py` lines 63–68 already list `Explain logP` and `解释分子生成的概念` with empty allowed/required tools. This is source evidence, not a test run or A1 approval. A2 narrows its history acceptance to these admitted prompts; it does not repair or bypass that classifier.
...
Accepted starts/resumes, including admission rejection and pre-model cancellation, deliver exactly one `agent_result` and one `complete` after owned work settles, provided the socket remains writable. This is a single-terminal safety rule, not a finite-time terminal promise for an uncooperative worker; see sections 6.1–6.2. Errors for malformed/unaccepted frames are not accepted turns. Add the same turn ID to decision-owned event/result/complete envelopes for stale-turn filtering. **Do not add fields to strict `molecule_candidates` envelopes**: those retain their existing exact schema, source trace and reference pointer, ordered before the enclosing turn's complete. Ignore frames from old sockets; ignore non-candidate frames for stale turns.
...
6. Disconnect/send timeout/overflow/server teardown cancel-and-drain all owned children; repeated cancellation cannot truncate cleanup. Existing `settle_action` shields only the outer Session work and must be extended with section 6.1 ownership settlement. Retain the lease through provider children, Session work, all nested real futures and executor joins. Preserve uncertain tool status and no replay. Cancelling an asyncio task, receiving a timeout result, or merely observing `future.done()` is not proof of complete thread cleanup.
7. Waiting turns release the lease after owned settlement and terminal delivery; their continuation record contains context/obligations but **no model/loop/client/task/worker-owner object**. Old clients may then close. Shutdown first stops new runtime admissions and cancels/drains active owners; only then may the existing app writer close unique model clients and registry owners. Do not hold writer while awaiting a request that still needs a reader. Repeated shutdown retains close-once behavior. If an owned thread never exits, graceful shutdown cannot complete: retain ownership and report unresolved cleanup, without closing resources still in use or claiming a clean shutdown.
...
### 6.1 Minimal nested-worker ownership revision

| Alternative | Cost and boundary | Decision |
|---|---|---|
| Per-turn worker ownership in the existing Agent runtime layer, instrumenting both real submit sites | One small helper plus narrow submit/settlement hooks; request isolation and original no-owner semantics remain explicit | **Parent-approved direction** |
| Unconditionally join both executors before every old timeout returns | Fewer bookkeeping changes, but changes legacy timeout latency and may wait forever | Not selected |
| Track only adapter futures or only the outer asyncio task | Smaller patch, but misses workflow descendants or physical thread lifetime | Insufficient; not an alternative proof of drain |

The proposed helper is `src/agent/runtime/worker_ownership.py`. It is an in-memory ownership ledger and settlement primitive, **not a new executor pool, scheduler, persistent task store or background framework**. The app/runtime retains the set of active turn owners; each owner retains only its own real `concurrent.futures.Future` and associated executor records. Do not use the shared `ToolRegistry` as a per-turn ledger or wait for unrelated requests sharing an adapter. Existing budgets/concurrency limits still apply; release settled records after cleanup and retain no growing history of completed workers.

Within that turn ledger, each `settle_action` call has an action-root scope for its outer Session call and nested workflow/adapter submissions. Action settlement closes and drains **that root's entire descendant tree**, not the still-running parent decision loop. Turn-wide settlement seals further action roots after dispatch ends and drains all remaining roots. This distinction prevents `settle_action` from waiting for the loop that is itself awaiting `settle_action`; it introduces no additional model lease.

Required mechanics for later implementation:

- Provide an optional server-only `worker_owner=None` through runtime/bridge/loop. The loop binds it to its request-local execution scope; `decision_execution.py` retains its outer Session task and joins both that task and the owner's descendants. Never store ownership on shared adapters, model instances, `AgentContext`, scientific results or continuations; browser fields cannot supply it.
- At **both** `WorkflowOrchestrator._execute_step` and `ToolAdapter._execute_once`, reserve ownership **before submission**. Attach the returned real future/executor to that reservation; executor creation/submission failure rolls the reservation back and preserves existing slot release/error behavior. A worker may finish or submit a child before `submit()` returns: reservation/attachment must still prevent a false empty ledger.
- Explicitly propagate/rebind the same owner and action-root lineage into each submitted thread callable and reset the binding on exit. A scoped context variable is an implementation option, but bare `ThreadPoolExecutor.submit` is not assumed to propagate it. Ownership travels outside tool arguments and outside scientific authority; existing schema validation, invocation slots and no-retry decision behavior remain intact.
- Action-root completion seals submissions from that root, **not** already-owned descendants registering their own children. Root-scoped `settle()` waits until the outer Session call has ended, no submission reservations remain in that tree, every descendant future has settled and every registered executor has joined. Only turn-wide settlement seals further action roots after decision dispatch ends. Descendants may register during draining; joining only an initial snapshot is forbidden. Repeated settle/cancel callers share cleanup rather than starting it twice.
- Future completion alone is insufficient for a thread-exit claim. Join each executor with `shutdown(wait=True)` away from the event-loop thread and away from that executor's own worker/done callback. Retain and drain any asynchronous join helper as owned cleanup; cancelling its asyncio wrapper does not prove the join completed. Completion callbacks may signal bounded bookkeeping, not prematurely release the lease.
- On timeout/cancel stop further model/tool dispatch. Preserve the original timeout/cancelled/unknown outcome; late success is cleanup information, not replacement evidence or permission to replay. Keep the reader lease until provider children, root work, descendant futures and executor joins all finish. Then perform single-terminal delivery if writable. A safe nonterminal settling indication is permissible, but no terminal is fabricated to unblock cleanup.
- With no owner, old callers retain their original timeout, return-value, retry and ownership behavior. This revision does not claim to repair all legacy or tool-internal detached work. If an admitted tool launches an untracked background consumer beyond these two submission paths, its lifecycle needs explicit scope review before a full-drain claim.

Future evidence must distinguish `worker_entered`, `timeout_returned`, `worker_exited`, `executor_joined`, `owner_settled`, reader release, epoch publication and resource close counts. Exercise adapter-first timeout, workflow-first timeout, delayed descendant registration, submission failure, parallel isolated owners, repeated cancellation and blocked executor join. Before worker/join barriers release: no owner removal, reader release, replacement epoch publication, resource close or final complete. After release: settled journal, unchanged failure/unknown truth, no replay, one terminal if writable and close-once. A never-exiting-worker scenario is observed with a controlled barrier and released during test cleanup; it proves the pending/unresolved boundary, not finite-time shutdown.

### 6.2 Explicit boundary with C's preparatory design

Parent reports that C requires a **finite-time UI terminal while retaining a cleanup owner**. This A2 revision instead waits for owned settlement before its terminal and **does not promise a finite-time terminal under an uncooperative worker**. Therefore C's terminal/cleanup split is a later integration-review difference, not compatibility established by this freeze. No C behavior, terminal deadline, detached cleanup handoff or new background mechanism is implemented or claimed here. Later C integration must review single-terminal ownership, late-frame suppression, lease/resource retention and shutdown semantics together; cancelling an asyncio task is never acceptable evidence of drain in either design.
...
For this batch, ordinary chat means **existing admitted greetings and closed nominal explanations**, not arbitrary free-form conversation. Replace the invented-label/recall scenario with `Explain logP` followed by `解释分子生成的概念`. Both turns must traverse actual A1 admission, loop and model adapter. If the first safe displayed answer is `A`, the second adapter request must contain the exact retained `user: Explain logP`, `assistant: A`, then current `user: 解释分子生成的概念`, with existing decision/protocol system prefixes unchanged. Capture `A` from the first response; do not prepopulate history, monkeypatch admission, or produce a canned runtime answer. An in-memory provider transport may script valid decision responses only at that external boundary. This proves precise pair transmission, **not real-model understanding/recall and not complete ordinary-chat functionality**. Arbitrary chat/classification expansion requires a separate design; an admission rejection is not a loop waiting result and must not receive a fabricated continuation nonce.
...
| **New** `src/agent/runtime/worker_ownership.py` | Per-turn real-future/executor ledger, reserve/rollback, cross-thread binding and full descendant/join settlement; no scheduler or persistent store |
| `src/agent/orchestrators/workflow.py` | Optional owned registration/propagation at existing `_execute_step` submit site; preserve no-owner path |
| `src/agent/tooling/adapters.py` | Optional owned registration/propagation at existing `_execute_once` submit site; preserve schemas, slots, retries and no-owner path |
| `src/agent/harness/decision_execution.py` | Retain outer Session task and drain owned descendants/executors under repeated cancellation |
| `src/agent/harness/decision_loop.py` | Seed admitted conversational history and bind optional worker ownership/settlement before finalization; no policy/tool expansion |
...
| **New** `tests/agent/test_worker_ownership.py` | Both nested submit paths, reserve/rollback, propagation, descendant races, executor joins, per-owner isolation and no-owner compatibility |
| `tests/agent/test_decision_loop.py` | Focused opt-in ownership/settlement regressions while retaining existing direct-caller behavior |
...
Read-only dependencies by default: A1 `decision_request.py`, `decision_bounds.py`, `decision_inputs.py`; `model_lifecycle.py`; session/auth modules; model adapters and config/key-store modules; scientific references, candidate normalization/controller, `ToolRegistry`, tool policies/domain adapters and HTTP workflow routes. The sole shared execution-layer exceptions are the two submit hooks and `decision_execution.py` listed above; they do not authorize other workflow/adapter refactors. If tests require changes outside this allowlist, parent must revise scope first. No production/test edits on the A1 worktree. No template/CSS redesign: create small accessible controls using the current home UI's DOM construction pattern.
...
- **Revised design text (parent-approved):** parent reviewed sections 6.1–6.2, plan Tasks 5–6 and the future allowlist, including action-root versus turn settlement. Nested real-worker ownership plus executor joins, no-owner compatibility and admitted-prompt exact history transmission are approved as design text. Only approval-status updates and an explicitly staged local commit of the two documents are authorized now. Section 6.2 still records the unresolved A2/C terminal-boundary difference, not a compatibility approval or scope change.
...
- **G2 Cross-contract revisions (pending):** target/property/candidate/reference or presentation changes after `bb11ded` require renewed scope review. Recheck the worker submission hooks observed at `9a9eb8d` and admitted prompts observed in dirty A1 against the landed integration base. History prefix changes must update run/fingerprint/replay/revision together. No widening A1 or B to make a route test pass. Later C integration must separately reconcile finite UI termination with retained cleanup ownership; this freeze does not resolve that difference.
