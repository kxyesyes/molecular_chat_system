...
## 23. Owned loopback HTTP exception in the ignored offline test harness

Configuration-fixture checkpoint is `453daf7a9d495d7e7631d21051fb529bd1e7a7c3`. The unrelated managed-process exact node passed once in 6.67s; its transparent probe failed once in 9.77s with the original five-second marker deadline. In that probe `Process.start()` took 0.033s and returned at 0.077s, child alive; at 5.018s the marker was absent, while cancellation cleanup completed cooperatively by 5.390s with child exitcode 0 and marker present. No force cleanup occurred. This identifies late observed readiness after spawn returned, not a specific import/scheduling/filesystem cause and not a passing normal worker-shutdown path. Preserve this UNKNOWN and all original deadlines; no Temporal code change is authorized here.

### Confirmed harness conflict and selected boundary

`tests/task_runtime/test_temporal_prometheus.py::test_metrics_server_lifecycle_shuts_down_and_propagates_port_conflicts` deliberately creates a real loopback Prometheus server and verifies HTTP response, port conflict, repeated shutdown, thread exit and successful rebinding. The ignored offline runner's blanket `socket.create_connection`/`connect` ban caused its failure. Actual server creation returns `MetricsServerHandle` with `_server`, `_thread` and `_endpoint`; `urlopen` uses both socket connection entry points, not `connect_ex`.

Choose an **exact-test, exact-owned-endpoint exception**, not unrestricted loopback/network access or a mocked HTTP response. The user delegates recommended choices. Only new ignored scratch helpers/probes plus later evidence in this plan are in scope; production, original tests, CI, default configuration and existing runners remain unchanged. No real external service or credential is involved.

Implementation files: `scratch/p7a2_owned_metrics_gate.py` (stdlib-only gate plus lazy pytest integration), `scratch/p7a2_owned_metrics_gate_test.py` (deterministic policy controls), `scratch/p7a2_owned_metrics_runner.py` (delegates the approved repository isolation with this one exception). Names are reserved for this sub-batch. Do not replace old probes/runners.

The gate has these concrete operations: `begin(nodeid)`, `register(handle)`, `revoke(handle)`, `allows(address)`, `end()`. It is disabled outside the one exact original test path/name; policy tests use separate instances with fake native functions and do not open sockets. `register` is called only by a wrapper that first delegates to the real successful `TemporalWorkerMetrics.start_loopback_server`; it must verify actual open socket `getsockname()`, handle endpoint, IPv4/TCP loopback address and live server thread. It retains that handle identity, not just a reusable port number. A failed/invalid start never grants access, must clean up a newly created owned handle if necessary, and must still fail.

Wrap real handle shutdown with `revoke(handle)` **before** delegating; errors propagate. Removing an old handle must not revoke a new handle at the same endpoint. Outside the original call, clear all permissions and restore the previous offline gate, never unrestricted native networking. Cleanup anomalies must fail, not be swallowed or represented as successful normal cleanup.

The new runner captures native `create_connection`/`connect` before installing the existing network denial. For the one active verified handle only, authorize the exact literal `('127.0.0.1', actual_port)`; validate again at the native IPv4/TCP connect boundary. Reject other ports, `localhost`, IPv6, other nodeids and external addresses **before DNS or any native connection**. `connect_ex` remains prohibited. Retain the original narrowly identified stdlib `_fallback_socketpair` exception unchanged; do not broaden it or break Windows asyncio by accidentally changing its caller check. Original pre-import OS whitelist, temp cwd/config/DB/cache, JSONL copies/SHA, normal conftest and failure reporter stay intact. Scratch-only policy execution must explicitly load the existing conftest without double-loading it for normal test paths.

### TDD and actual integration sequence

- [ ] First add deterministic policy tests and observe RED before implementing the gate: disabled/unowned endpoint, wrong address/port/nodeid, closed or shutting-down handle, failed registration, same-port replacement identity and idempotent revocation. Inject native callbacks that count attempts and assert zero calls/DNS for denied cases; these are policy checks, not actual-server proof.
- [ ] Implement only the bounded ignored gate/plugin/runner above. Lazy-import application modules only after approved pre-import isolation and normal conftest. Verify all negative policy controls GREEN; do not relax network bans to make a control pass.
- [ ] Run the **unchanged exact original metrics lifecycle test once** through the new runner. All original real HTTP/body, conflict, shutdown, thread-exit and rebinding assertions must execute; additionally verify permissions empty and offline bans restored afterward. Preserve the earlier harness failure in the report.
...
### Complete affected-root confirmation

At clean `95405161e1d1bbaaca70843e72ad72589f328875`, tree `d1f33f75cc24382d588851b59107e1411049c7f2`, the full root non-activity partition ran once: **2060 passed / 0 failed / 143 skipped / 173 passed subtests / 5 warnings / 324.08s**, exit 0. Session 12239 ended; no Python/pythonw processes remained. The 2203 main-case total is one above the original 2202, matching the added configuration-fixture regression. Existing platform/permission/tool/opt-in skips and PyG/FastAPI deprecation warnings remain visible.

Command: MedChat Python `-B -X utf8 scratch/p7a2_repository_offline_runner.py <absolute-tests>` followed by separate absolute `--ignore` paths for `tests/agent`, `tests/sandbox_broker`, `tests/task_runtime`, and all 30 `tests/test_activity*.py` files (enumerated by filename and count-checked before execution). Original pytest options, OS-whitelisted isolation, temporary configuration/storage/cache, network denial and three JSONL SHA checks are unchanged. Repository runner SHA `22c33223a24ddd30d473989d03b0ea4e188d7db6aecab648540223df470a9803`, isolation runner SHA `9a52048381b29d9d3b2e1fbb0e44e33adfb6802caf5164892ceca9e6dea9af4c` and safe failure reporter SHA `f94e26c310f3cf6e6a09f6d1e4cd1e59a0592e9faa74f306ad580d173914265c` are unchanged. No rerun or code/assertion/deadline adjustment occurred. Full Agent confirmation and CI remain pending; earlier failures are not erased.

## 26. Complete Agent confirmation and final source reviews

The entire Agent partition ran once on `dd14409471fe3b631f27419520a7163b219902f8`, tree `5d15b7741c222eccbc135e5f6f3ce973f206a5aa`:

```powershell
& C:/Users/xkx52/.conda/envs/MedChat/python.exe -B -X utf8 scratch/p7a2_repository_offline_runner.py D:/MedChat/molecular_chat_system_worktrees/web-decision-runtime-integration/tests/agent
```

Actual result: **8114 passed / 0 failed / 2 skipped / 7 warnings / 687.44s**, exit 0, session 93262 ended. Total 8116 cases; no prefix filter, deselection, changed deadline, extra instrumentation, restart or repeated full run. Both skips retain existing directory-symlink/performance opt-in conditions; warnings are SWIG/FastAPI deprecations. Normal conftest, approved isolation and all three JSONL SHA checks remain. All owned sessions ended, with no remaining Python/pythonw processes.

Galileo independently approves final SOURCE/SPEC and Peirce final SOURCE/QUALITY for that exact 27-file snapshot. Production `src/` remains identical to `2769e1eca86220c5c3be3a749392148133979d53`; the only later test addition is the separately reviewed root fixture blob `6c8612ab29178881f06754048cb844c101a6cb4e`. Original fixture assertions, scientific contracts, compatibility behavior and ownership/cleanup boundaries remain. No ignored helpers or runtime assets are tracked; diff checks pass. Documentation retains UNKNOWN failures and does not claim P7/P8 complete.

**687.44s exceeds CI's 600-second Agent command budget.** This local pass is not an equivalent CI performance pass, does not establish the earlier five guard timeouts' cause, and does not justify changing any timeout. All six local partitions now have recorded passing executions on the reviewed code/test boundaries (unchanged sandbox and activity partitions retain section-21 results), but warnings/skips and local-versus-CI differences remain explicit. Remote head/branch state, a unique A2 PR, all eight CI checks and final reviewed-tree/merge-tree verification are still required before merging. Ordinary-chat/B/C integration and P8 live acceptance remain subsequent tasks; no model activation, publication or deployment occurred in these confirmations.
