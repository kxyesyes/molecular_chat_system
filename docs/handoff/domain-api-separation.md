...
PR #64 was opened at `6dd92db1b1d3c77fdd8bded15065c662486694c2`. Its first CI run `36048361209` failed when the root suite reached the existing 600-second process deadline (exit124, around89%); it did not show a test assertion failure before termination. This failed run remains part of the evidence. The isolated local root profile completed separately: **3559 passed, 153 skipped, 6 warnings, 173 subtests passed in701.12s**. That local result does not make the timed-out CI run a pass.

Independent infrastructure PR #65 partitions root and top-level activity tests using exact collected paths. It preserves assertions, per-test deadlines and the existing600-second deadline for each job, while transparently increasing the combined root budget from600 to1200seconds. Actual collection verified3646=1785+1861 node IDs, no intersection or duplicates, and an identical union. Its first run failed a second workflow-count assertion; after that test-only correction and independent re-review, run `36052914906` passed8/8 checks. It squash-merged as `68b85388bd3b9f072f9e74e51ab1e61f75b45b1e`; reviewed and merged trees match.

That reviewed main revision was merged into this branch without conflicts as `8ee8350afcc201c89e342d30b3dfe26865e9b0ec`. No API production code changed in the integration. Post-integration isolated command: `python -B -m pytest tests/test_api_route_boundary.py tests/test_quality_workflow_contract.py tests/test_deployment_assets.py -q -p no:cacheprovider --tb=short -rs`, using the same documented runner: **85 passed, 7 warnings in8.73s**, exit0. A fresh exact-head8-check CI run and review-state check are still required before PR #64 can merge.
