...
PR #22 初始提交 `f4337ef`，CI run34692710150 汇总失败，未重跑抹去失败：

- Agent job103550700639：2583 passed、1 failed、1 skipped；near_budget 将 75 次 520,000 字符扫描共用 10 秒截止时间，触发子进程超时。
- sandbox-core job103550700590：1927 passed、1 failed、4 skipped；既有 late cancellation create 测试在 0.5 秒等待 terminal 时超时，独立分支诊断，不能归因于本批存储改动或直接视为已修复。
- root、task-runtime、sandbox-api、static-quality 通过。

父任务复测原三个扫描测试为 2.55/8.71/4.81 秒，near_budget 在开发机已接近 10 秒，不能把单次扫描复杂度与 75 次矩阵总耗时混为同一门禁。
仅调整测试：每组完整 75 个末尾凭据按 `tails[group::5]` 分成 5 个不交叠批次，每批 15 个，仍保留每个子进程 10 秒截止时间、同样最大输入、有效完整快照和末尾凭据拒绝；不减少覆盖或修改生产扫描器。
新 15 项初次全部通过，最慢 2.17 秒，总 21.29 秒。
独立规格审查确认 225/225 组合保留；每次调用注入 50 ms 延迟时旧全集在 10.031 秒被终止，
新批次 2.756–2.818 秒通过。恢复旧平方级扫描仍被 10 秒门禁终止，16 KiB 截断/恒 false/移除 URL 检测等变异均被拒绝。
独立质量审查正式文件 318 passed、独立探针 12 passed；确认超时子进程回收和输出管道关闭，无可操作意见。
父任务联合回归 **2686 passed、1 skipped、7 warnings，65.36s**。新 CI 尚待运行，独立 sandbox 问题仍未宣称修复。
