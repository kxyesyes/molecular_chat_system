...
| 真实权重全链路测试 | `9312bf5`，tests/test_activity_family_real_acceptance.py | 方案A实施中，Task1–5已双审通过，Task6报告独立SPEC发现3项缺口正在修复；尚未创建本批PR。进度与实际测试见[交接](family-real-acceptance-integration.md)。真实目录和精确bundle另行配置授权，不把合成前向当真实模型验收。 |
