...
TRAINING_DATA_CANDIDATES = ("chembl_data_with_fps.tsv", "chembl_training_data.tsv")
...
def inspect_reverse_target_database(data_dir: str | Path = "data/reverse_target") -> dict[str, Any]:
    ...
    training_data_path = _resolve_training_data_path(root)
    files = {"training_data": _file_state(training_data_path)}
    files.update(
        {
    ...
    record_count = _read_cached_record_count(root)
    if record_count is None:
        record_count = _count_tsv_records(training_data_path) if files["training_data"]["exists"] else 0
...
def _resolve_training_data_path(root: Path) -> Path:
    for filename in TRAINING_DATA_CANDIDATES:
        path = root / filename
        if path.exists():
            return path
    return root / TRAINING_DATA_CANDIDATES[0]
...
def _read_cached_record_count(root: Path) -> int | None:
    path = root / "reverse_target_metadata.json"
    if not path.exists():
        return None
    try:
