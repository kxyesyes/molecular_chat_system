...
(function () {
  ...
  function init() {
    ...
    elements.themeButtons = document.querySelectorAll(".theme-chip");
  ...
  // 创建聊天容器
  function applyHomeTheme(theme, persist = true) {
    const allowedThemes = ["rose", "amber", "mint"];
    const nextTheme = allowedThemes.includes(theme) ? theme : "rose";
    ...
    if (persist) {
      localStorage.setItem("medchatHomeTheme", nextTheme);
    }
  }

  function initThemeSwitcher() {
    const savedTheme = localStorage.getItem("medchatHomeTheme") || "rose";
    applyHomeTheme(savedTheme, false);

    if (!elements.themeButtons || !elements.themeButtons.length) return;

    elements.themeButtons.forEach((button) => {
      button.addEventListener("click", () => {
        applyHomeTheme(button.dataset.theme);
      });
    });
  }
  ...
  function createChatContainer() {
    ...
    // 插入到合适的位置
    const topElement = rightPanel.querySelector(".top");
    rightPanel.insertBefore(chatMessages, topElement.nextSibling);
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        if (!event.wasClean && reconnectAttempts < maxReconnectAttempts) {
          ...
          showNotification(`连接断开，${delay / 1000}秒后重连`, "warning");
      ...
      updateConnectionStatus("error");
      showNotification(`连接创建失败: ${error.message}`, "error");
    }
  }
    ...
    try {
      ...
      switch (message.type) {
        case "connection_ready":
          ...
          showNotification("模型已就绪", "success");
          break;
  ...
  async function handleModelChange(e) {
    ...
    try {
      ...
      if (result.success) {
        ...
        // 显示Toast通知
        showToast(`已切换到 ${getModelDisplayName(selectedModel)}`, "success");

        // 如果在聊天模式，也添加系统消息
        if (chatMode) {
          addSystemMessage(`已切换到 ${getModelDisplayName(selectedModel)}`);
        }
      } else {
        ...
        // 显示错误Toast
        showToast(`模型切换失败: ${result.message}`, "error");

        // 如果在聊天模式，也添加系统消息
        if (chatMode) {
          addSystemMessage(`模型切换失败: ${result.message}`, true);
        }
      }
    } catch (error) {
      ...
      // 显示错误Toast
      showToast("模型切换失败，请检查网络连接", "error");

      // 如果在聊天模式，也添加系统消息
      if (chatMode) {
        addSystemMessage("模型切换失败，请检查网络连接", true);
      }
    }
  }
  ...
  function sendMessage() {
    ...
    if (!ws) {
      ...
      showNotification("连接不存在，正在重新连接...", "error");
      connectWebSocket();
      return;
    }
    ...
    if (ws.readyState !== WebSocket.OPEN) {
      ...
      showNotification("连接未就绪，正在重新连接...", "error");
  ...
  function appendToLastMessage(content) {
    ...
    messageContent.innerHTML = formatContent(newContent);
  ...
  function formatContent(content) {
    // 用户要求：回答时不显示 Markdown 符号（##、**、```）
    // 这里做统一清洗，避免这些标记出现在 UI 上
  ...
  function toggleRAG() {
    ...
    updateToggleStates();
    showNotification(`RAG检索已${ragEnabled ? "启用" : "禁用"}`, "info");
  ...
  function toggleTools() {
    ...
    updateToggleStates();
    showNotification(`智能工具已${toolsEnabled ? "启用" : "禁用"}`, "info");
  ...
  function handleQuickAction(event) {
    ...
    const quickMessages = {
      分析乙醇性质: "请分析乙醇(CCO)的分子性质和ADMET特性",
      生成类药分子:
        "请生成符合Lipinski五规则的类药分子，分子量小于500，LogP小于5",
      ADMET预测: "请预测阿司匹林(CC(=O)Oc1ccccc1C(=O)O)的ADMET性质",
    };

    const message = quickMessages[text];
    if (message) {
  ...
  function displayRAGInfo(molecules, infoMessage) {
    ...
    // 根据数量确定卡片样式参数
    let minCardWidth, imageHeight, gap, itemsPerPage;

    if (moleculeCount <= 2) {
      minCardWidth = "320px";
      imageHeight = "200px";
      gap = "24px";
      itemsPerPage = moleculeCount;
    } else if (moleculeCount <= 6) {
      minCardWidth = "260px";
      imageHeight = "180px";
      gap = "20px";
      itemsPerPage = 6;
    } else if (moleculeCount <= 12) {
      minCardWidth = "220px";
      imageHeight = "150px";
      gap = "16px";
      itemsPerPage = 9;
    ...
    // 分页逻辑
    let currentPage = 0;
    const totalPages = Math.ceil(moleculeCount / itemsPerPage);
    const needsPagination = totalPages > 1;

    const moleculesContainer = document.createElement("div");
    moleculesContainer.className = "rag-molecules-grid";
        ...
        allProperties.forEach((prop) => {
          ...
          `;
            ...
            if (prop === "qed") {
              ...
              valueColor =
                propValue >= -0.4 && propValue <= 5.6 ? "#059669" : "#dc2626";
            } else if (prop === "molwt") {
              valueColor =
                propValue <= 500
                  ? "#059669"
            ...
            <span style="color: ${valueColor}; font-weight: 600; font-size: 13px;">${
              prop === "molwt" ? propValue.toFixed(1) : propValue.toFixed(3)
            }</span>
          `;
            infoSection.appendChild(propRow);
          }
        });
    ...
    // 添加到聊天容器
    elements.chatContainer.appendChild(ragInfoContainer);
    scrollToBottom();
  }
      ...
      if (value !== undefined && value !== null) {
        ...
        html += `
          <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px dashed #e2e8f0; padding-bottom: 8px;">
            ...
            <span style="color: ${valueColor}; font-weight: 600; font-size: 13px;">${displayValue}${unit}</span>
          </div>
        `;
      }
    });
  ...
  function detectAndRenderMolecules(messageElement, content) {
    ...
    if (moleculeCount <= 2) {
      ...
      imageHeight = "200px";
      gap = "24px";
      itemsPerPage = moleculeCount; // 不分页
    } else if (moleculeCount <= 6) {
      minCardWidth = "260px";
      imageHeight = "180px";
      gap = "20px";
  ...
  function initAdvancedOptions() {
    ...
    if (!moreBtn || !panel || !overlay) {
      console.error("❌ 高级选项面板元素未找到", { moreBtn, panel, overlay });
      return;
    }
    ...
    // RAG检索数量滑块
    if (ragCountSlider) {
      ragCountSlider.addEventListener("input", (e) => {
    ...
    if (resetBtn) {
      resetBtn.addEventListener("click", () => {
        console.log("🔄 重置高级选项为默认值");
        advancedConfig.ragCount = 5;
        advancedConfig.temperature = 0.7;
        advancedConfig.molCount = 5;
        updateSliderValues();
        saveAdvancedOptions();
  ...
  function loadAdvancedOptions() {
    try {
      ...
      if (saved) {
        ...
        advancedConfig.ragCount = loaded.ragCount || 5;
        advancedConfig.temperature = loaded.temperature || 0.7;
        advancedConfig.molCount = loaded.molCount || 5;
        console.log("📂 已加载保存的高级选项:", advancedConfig);
      }
    } catch (error) {
